...
if(isset($_SESSION['user_id'])){
    ...
    if($_GET['data'] != '' & $_GET['data']!= null){
        ...
        if($data->status == "COMPLETE"){
            ...
            header('Location: paypal-method?error=redirect_incomplete');
            exit();
        }
    }else{
        header('Location: paypal-method?error=redirect_no_data');
        exit();
    }
    ...
    // create order 
    $order_sql = "INSERT INTO orders (`order_id`, `total`, `subtotal`, `vat_exempt_subtotal`, `discount`, `discounted`, `paid`, `payment_method`, `delivery_option`, `shipping_fee`, `user_id`) VALUES ('$order_id', '$total', '$subtotal', '$vat_exempt_subtotal', '$discount_total', '$discounted', '1', 'paypal', '$delivery_option', '$shipping_cost', '$user_id')"; 
    ...
    if($order_stmt){
        ...
        if($orders_products_stmt){
            ...
            if($delete_stmt){
                ...
                // $data = json_decode($_GET['data']);
                // $user_id = $_SESSION['user_id'];
                // insert in paypal table 
                $paypal_sql = "INSERT INTO paypal_data (`data`, `order_id`, `user_id`) VALUES ('$raw_data', '$order_id', '$user_id')"; 
                $paypal_stmt = mysqli_query($conn, $paypal_sql);

                if($paypal_stmt){
                    echo 'success insert paypal data';
                    header('Location: place-order?method=paypal&id='.$order_id);
                    exit();
                }else{
                    echo 'failed to insert paypal data';
                    exit();
                }
            }else{
