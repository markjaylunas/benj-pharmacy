...
if(isset($_SESSION['user_id'])){
    ...
    //     }
    //     else{
    //         header('Location: gcash?error=redirect_incomplete');
    //         exit();
    //     }
    // }else{
    //     header('Location: gcash?error=redirect_no_data');
    //     exit();
    // }
    ...
    // create order 
    $order_sql = "INSERT INTO orders (`order_id`, `total`, `subtotal`, `vat_exempt_subtotal`, `discount`, `discounted`, `paid`, `payment_method`, `delivery_option`, `shipping_fee`, `user_id`) VALUES ('$order_id', '$total', '$subtotal', '$vat_exempt_subtotal', '$discount_total', '$discounted', '1', 'gcash', '$delivery_option', '$shipping_cost', '$user_id')"; 
    ...
    if($order_stmt){
        ...
        if($orders_products_stmt){
            ...
            if($delete_stmt){
                ...
                $gcash_sql = "INSERT INTO gcash_data (`data`, `order_id`, `user_id`) VALUES ('$raw_data', '$order_id', '$user_id')"; 
                $gcash_stmt = mysqli_query($conn, $gcash_sql);

                if($gcash_stmt){
                    echo 'success insert gcash data';
                    header('Location: place-order?method=gcash&id='.$order_id);
                    exit();
                }else{
                    echo 'failed to insert gcash data';
                    exit();
                }
            }else{
